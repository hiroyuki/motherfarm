...
#ifndef motherfarmLED_GradualChangerState_h
#define motherfarmLED_GradualChangerState_h
#include "BaseState.h"
class GradualChangerState : public BaseState
{
public:
    int longestLen;
    ...
    GradualChangerState(SharedData *sharedData):BaseState(sharedData)
    {
        gui.addToggle("GradualChangerState", isActive);
    }
    ...
    void update()
    {
        brightness = sharedData->curveA*200.f;
        BaseState::update();
        ...
        for( int i = 0; i < SVG_WIDTH; i++)
        {
            if ( compw - floor( compw ) < compressw)
            {
                for( int j = 0; j < SVG_HEIGHT; j++)
                {
                    ...
                    {
                        colorPixels->getPixels()[(i+j*SVG_WIDTH)*4] = ofNoise(i*0.005+t*0.0002, j*0.005+t*0.0002) * brightness *alpha;
                        colorPixels->getPixels()[(i+j*SVG_WIDTH)*4+1] = ofNoise(i*0.005-t*0.0002, j*0.005+t*0.0002) * brightness *alpha;
                        colorPixels->getPixels()[(i+j*SVG_WIDTH)*4+2] = ofNoise(i*0.005-t*0.0002, j*0.005-t*0.0002) * brightness * alpha;
